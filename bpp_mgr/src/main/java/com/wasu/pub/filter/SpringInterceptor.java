...
public class SpringInterceptor  implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		Object URM_SESSION =  request.getSession().getAttribute(UrmService.URM_SESSION);
		Object USER_SESSION = request.getSession().getAttribute("loginUser");
		String contextRoot = request.getContextPath();
		String path = request.getRequestURI();
		if (path.contains(contextRoot))
		{
			path = path.substring(contextRoot.length());
		}
		
		if (URM_SESSION == null || USER_SESSION == null)
		{
			if(path.endsWith(".js") || path.endsWith(".css") || path.endsWith(".gif") || path.endsWith(".jpg") || path.endsWith(".png") || path.endsWith(".jsp")) return true;
			/** 登录登出页面不进行拦截 */
			if (loginURL.equals(path) || loginSpringUrl.equals(path) || loginStrutsUrl.equals(path) || logoutSpringUrl.equals(path) || logoutStrutsUrl.equals(path))  return true; 
			if (request.getHeader("x-requested-with") != null && request.getHeader("x-requested-with").equalsIgnoreCase("XMLHttpRequest")){ 
                       ...
                       complete:function(XMLHttpRequest,textStatus){ 
                         var sessionstatus=XMLHttpRequest.getResponseHeader("sessionstatus"); //通过XMLHttpRequest取得响应头，sessionstatus，  
                             if(sessionstatus=="timeout"){ 
                                 alert("登录超时,请重新登录！");
                             //如果超时就处理 ，指定要跳转的页面  
                               window.location.replace(webPath.webRoot + "/views/login.jsp");   
                             }   
                        }   
            	});
            	</script>
            	*/
                ...
                response.sendRedirect(contextRoot+loginURL);
            }
			//这里为false,暂时改成true
			return false;
		}
